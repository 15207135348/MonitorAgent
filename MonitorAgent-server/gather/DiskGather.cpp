...
bool DiskGather::compute()
{
    ...
    m_usage[2] = (double)(m_value[1][2] - m_value[0][2])/m_sec;
    m_usage[3] = (double)(m_value[1][3] - m_value[0][3])/m_sec;

    return true;
}
double DiskGather::ikbs()
{
    return m_usage[0];
}
double DiskGather::okbs()
{
    return m_usage[1];
}
double DiskGather::ips()
{
    return m_usage[2];
}
double DiskGather::ops()
{
    return m_usage[3];
}
//获取剩余磁盘空间
